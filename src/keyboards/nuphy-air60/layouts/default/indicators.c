...
// The effect maths live in src/smk/led_effect.{h,c}; the per-board geometry
// tables are generated by utils/led_geometry_gen.c (a ROM table, no RAM cost).
// This file only declares the matrix size so the sizes can be cross-checked,
// then renders pixels. The generated header is selected per board via meson.
...
bool indicators_update_step(keyboard_state_t *keyboard, uint8_t current_step)
{
    ...
    // Parking every subframe also discharges the columns thoroughly, so the old
    // once-per-frame blanking tick (which only covered row 0) is gone.
...
void indicators_pwm_disable()
{
    ...
    // — is what lets the per-subframe re-enable come back glitch-free (the slaves
    // were left at 0 before, which glowed on re-arm).
    PWM00CON = 2;
    PWM01CON = 2;
    PWM02CON = 2;
    PWM03CON = 2;
