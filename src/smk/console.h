...
// by design so it fits where SDCC's printf_large does not: the bulk storage is
// console_buf (__xdata) and the formatter keeps only transient state on the
// stack. Supports %%, %c, %s, and %d/%u/%x/%X with an optional '0' flag + single
// width digit. 16-bit range. __reentrant, so ISR-safe. This is the single debug
// output path -- everything routes through dprintf()/console_printf.
