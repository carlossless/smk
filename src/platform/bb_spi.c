...
// Stock-faithful open-drain SPI: SCK/MOSI/CS/MOT idle as INPUT with
// pull-up (high). To drive LOW, we briefly switch the pin's PxCR
// direction bit to OUTPUT while the latch holds 0. To release HIGH, we
// switch back to INPUT and let the pull-up take it.
//
// Stock decompile reference: bb_spi_xfer_entry @ CODE:0x9cda for the
// MOT/CS toggles; bb_spi_byte_tx @ CODE:0xB07D and bb_spi_byte_rx @
// CODE:0xB16D for the per-bit SCK/MOSI toggles.
#define MOT_DRIVE_LOW()    do { RF_BB_SPI_MOT = 0; P0CR |= _P0_5;  RF_BB_SPI_MOT = 0; } while (0)
#define MOT_RELEASE_HIGH() do { P0CR &= (uint8_t)~_P0_5; RF_BB_SPI_MOT = 1; } while (0)
#define CS_DRIVE_LOW()     do { RF_BB_SPI_CS = 0;  P7CR |= _P7_4;  RF_BB_SPI_CS = 0; } while (0)
#define CS_RELEASE_HIGH()  do { P7CR &= (uint8_t)~_P7_4; RF_BB_SPI_CS = 1; } while (0)
...
static void bb_spi_burst(uint8_t *data, int len, bool lock)
{
    ...
    // input (pull-up high) to match stock's exit state.
    P0CR &= (uint8_t)~_P0_7;
    RF_BB_SPI_MOSI = 1;
    MOT_RELEASE_HIGH();
}
...
void bb_spi_recv(uint8_t *data, int len)
{
    ...
    // No ACK poll — stock's RX (rf_get_status at CODE:0x9108) doesn't.
}
...
uint8_t bb_spi_xfer_byte(uint8_t data)
{
    ...
    // Per-bit open-drain bit-bang (stock-faithful):
    //
    ...
    //
    // Stock decompile reference: bb_spi_byte_tx CODE:0xB07D, bb_spi_byte_rx
    // CODE:0xB16D. The MISO sample happens while SCK is LOW, matching
    // stock's bb_spi_byte_rx pattern (slave latches MISO on SCK falling
    // edge, master reads while low, then releases SCK high for the next
    // slave latch).
