...
class UcsimSession:
    ...
    def get_sfr(self, addr):
        # Reply is the echoed command then the data line, e.g.
        #   dump sfr 0x88 0x88
        #   0x88 TCON:            0b00000000 0x00 '.'   0
        # (uCsim labels SFRs with the stock 8051 names; on the SH68F90 0x88 is
        # actually P5 -- only the value matters.) Skip the echo, then take the
        # "0x<HH>" value that follows the binary field.
        # uCsim labels SFRs it knows (stock 8051 names) with a verbose line and
        # dumps the rest bare:
        #   0x88 TCON:            0b00000000 0x00 '.'   0     <- named
        #   0xf8                  00 .                        <- bare (P7)
        # (On the SH68F90 0x88 is really P5, 0xf8 is P7 -- only the value
        # matters.) Skip the echo, then read the named "0x<HH>" after the binary
        # field, falling back to the bare hex byte after the address.
