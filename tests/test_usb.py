...
class TestKeyEvent(unittest.TestCase):
    ...
    press propagates through the real scan path -- the PWM/Timer2 ISR drives
    matrix_scan_full(), the main loop's matrix_task() detects the change and
    calls process_key_state(), which resolves the keycode from the real keymap
    and (in USB mode) sends the HID report on EP1.

    The key matrix is the *board's* hardware, so it is emulated test-side
    (devices.KeyMatrix): the simulator just exposes the GPIO ports, and the test
    pulls the pressed key's row low while its column is scanned. See devices.py."""
